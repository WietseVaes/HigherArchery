...
# Error displays
@printf "my HODLR: rel. normed err. %.5f\n" (norm(A_HODLR - A) / norm(A))
@printf "HODLR: rel. normed err. %.5f\n" (norm(A_HODLR_paper - A) / norm(A))
...
p1 = surface(abs.(A_HODLR - A), color=:jet, zscale=:log10,
             title=@sprintf("my HODLR: rel. normed err. %.5f", norm(A_HODLR - A) / norm(A)))
xlims!(1, size(A, 1))
ylims!(1, size(A, 2))
